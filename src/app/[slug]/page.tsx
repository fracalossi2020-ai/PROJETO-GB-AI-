...
export default function CardapioPage() {
  ...
  /* ── Active category on scroll (throttled) ── */
    ...
    <div className={`min-h-screen ${bg} ${text}`}>
      ...
      {toastMsg && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[60] animate-fadeInUp">
          <div className="bg-[#ff9607] text-black px-5 py-3 rounded-2xl font-bold text-sm shadow-lg whitespace-nowrap">
            {toastMsg}
          </div>
        </div>
      )}
      ...
      <div className="relative">
        {store.coverImage ? (
          <div className="absolute inset-0">
            <img src={store.coverImage} alt="" className="w-full h-full object-cover opacity-30" loading="eager" />
            <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-b from-[#050505]/40 via-[#050505]/70 to-[#050505]' : 'bg-gradient-to-b from-[#f8f7f4]/40 via-[#f8f7f4]/70 to-[#f8f7f4]'}`} />
          </div>
        ) : (
        ...
        <div className="relative px-4 pt-5 pb-5">
          <div className="max-w-3xl mx-auto flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <Link href={`/${slug}/meus-pedidos`} className={`${card} rounded-xl p-2 hover:scale-105 transition-transform`}>
                <ShoppingCart className="h-5 w-5" />
              </Link>
              <Link href={`/${slug}/rastrear`} className={`${card} rounded-xl p-2 hover:scale-105 transition-transform`}>
                <MapPin className="h-5 w-5" />
              </Link>
            </div>
            <div className="flex items-center gap-2">
              {isAuth && (
                <button onClick={() => router.push('/dashboard')} className={`${card} rounded-xl p-2 hover:scale-105 transition-transform`}>
                  <Sparkles className="h-5 w-5" />
                </button>
              )}
              <button onClick={toggleTheme} className={`${card} rounded-xl p-2 hover:scale-105 transition-transform`}>
          ...
          <div className="max-w-3xl mx-auto text-center">
            {store.logo ? (
              <img src={store.logo} alt={store.name} className="w-20 h-20 rounded-3xl mx-auto mb-3 object-cover ring-4 ring-[#ff9607]/20 shadow-xl" loading="eager" />
            ) : (
              <div className="w-20 h-24 bg-gradient-to-br from-[#ff9607] to-[#ff0080] rounded-3xl mx-auto mb-3 flex items-center justify-center text-black text-3xl font-black shadow-xl">
                {store.name.charAt(0)}
              </div>
            )}
            <h1 className="text-3xl font-black tracking-tight">{store.name}</h1>
            <p className={`${muted} mt-1 text-sm max-w-md mx-auto`}>{store.description}</p>

            <div className="flex items-center justify-center flex-wrap gap-2 mt-3">
              <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${card}`}>
                <Clock className="h-3.5 w-3.5 text-[#ff9607]" /> {store.deliveryTimeMin || 25}-{store.deliveryTimeMax || 45} min
              </span>
              <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${card}`}>
                <Star className="h-3.5 w-3.5 text-[#ff9607]" /> 4.9
              </span>
              <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${card}`}>
                R$ {store.deliveryFee?.toFixed(2) || '5.00'} entrega
              </span>
              <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${store.isOpen ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
            ...
            {/* Search */}
            <div className={`mt-4 max-w-md mx-auto flex items-center gap-2 ${inputCls} rounded-2xl px-4 py-2.5`}>
      ...
      {!search && featuredProducts.length > 0 && (
        <div className="max-w-3xl mx-auto px-4 mt-2 mb-3">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar snap-x">
            {featuredProducts.map((p: Product) => (
              <button
                ...
                className={`snap-start flex-shrink-0 w-36 ${card} rounded-2xl overflow-hidden text-left hover:scale-[1.02] transition-transform`}
              >
                <div className="h-28 relative">
                  ...
                  {p.isPromotion && <span className="absolute top-2 left-2 bg-[#ff0080] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">PROMO</span>}
                </div>
                <div className="p-2.5">
                  <h3 className="font-bold text-sm truncate">{p.name}</h3>
                  <p className="text-[#ff9607] font-bold text-sm mt-0.5">
      ...
      {/* ── Category Tabs ── */}
      <div className={`sticky top-0 ${stickyCat} z-30 border-b backdrop-blur-sm`}>
        <div className="flex gap-2 overflow-x-auto px-4 py-2.5 max-w-3xl mx-auto no-scrollbar">
          {categories.map(cat => (
            <button
              key={cat.id}
      ...
      {/* ── Products ── */}
      <div className="max-w-3xl mx-auto px-4 mt-5 pb-28 space-y-8">
        {isLoaded ? (
            ...
            filteredCategories.map((cat) => (
              <div key={cat.id} id={cat.id} ref={el => { sectionRefs.current[cat.id] = el; }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-xs ${muted}`}>{cat.products.length} itens</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {cat.products.map(product => (
                    <button
                      ...
                      className={`text-left ${card} rounded-2xl overflow-hidden group hover:shadow-md transition-shadow`}
                    >
                      <div className="relative h-40 overflow-hidden">
                        {product.image ? (
                          <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                        ) : (
                          <div className="w-full h-full bg-gradient-to-br from-[#ff9607]/10 to-[#ff0080]/10 flex items-center justify-center">
                            <span className="text-3xl font-black text-[#ff9607]/20">{product.name.charAt(0)}</span>
                          </div>
                        )}
                        <div className="absolute top-2 left-2 flex gap-1">
                          {product.isPromotion && <span className="bg-[#ff0080] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">PROMO</span>}
                          {product.isFeatured && <span className="bg-[#ff9607] text-black text-[10px] font-bold px-2 py-0.5 rounded-full">TOP</span>}
                        </div>
                        <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="w-9 h-9 rounded-full bg-[#ff9607] flex items-center justify-center shadow-lg">
                      ...
                      <div className="p-3.5">
                        ...
                        <p className={`${muted} text-xs mt-0.5 line-clamp-2`}>{product.description}</p>
                        <div className="mt-2 flex items-end justify-between">
                          {product.isPromotion && product.promotionPrice ? (
                            <div className="flex items-center gap-2">
                              <span className="text-base font-black text-[#ff9607]">R$ {product.promotionPrice.toFixed(2)}</span>
                              <span className={`text-xs line-through ${muted}`}>R$ {product.price.toFixed(2)}</span>
                            </div>
                          ) : (
                            <span className="text-base font-black text-[#ff9607]">R$ {product.price.toFixed(2)}</span>
                          )}
                          {product.addons.length > 0 && <span className={`text-[10px] ${muted}`}>+{product.addons.length} extras</span>}
                        </div>
                      </div>
                    </button>
                  ))}
          ...
          /* Skeleton - simple, no pulse */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[1,2,3,4].map(i => (
              <div key={i} className={`${isDark ? 'bg-white/[0.03]' : 'bg-white'} border border-black/[0.06] rounded-2xl overflow-hidden`}>
                <div className={`h-40 ${isDark ? 'bg-white/[0.04]' : 'bg-black/[0.04]'}`} />
          ...
          <div onClick={e => e.stopPropagation()} className={`${modalBg} w-full max-w-lg rounded-t-3xl max-h-[90vh] overflow-auto border-t border-white/[0.08]`}>
            {selectedProduct.image && (
              <div className="relative h-52">
                ...
                <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/50 flex items-center justify-center">
                  <X className="h-5 w-5" />
                </button>
              </div>
            )}
            <div className="p-5 space-y-4 -mt-3 relative">
              <div className="flex justify-between items-start gap-3">
                <div>
                  <h2 className="text-xl font-black">{selectedProduct.name}</h2>
                  <p className={`${muted} text-sm mt-0.5`}>{selectedProduct.description}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  {selectedProduct.isPromotion && selectedProduct.promotionPrice ? (
                    <div>
                      <span className="text-xl font-black text-[#ff9607]">R$ {selectedProduct.promotionPrice.toFixed(2)}</span>
                      <span className={`block text-xs line-through ${muted}`}>R$ {selectedProduct.price.toFixed(2)}</span>
                    </div>
                  ) : (
                    <span className="text-xl font-black text-[#ff9607]">R$ {selectedProduct.price.toFixed(2)}</span>
                  )}
                </div>
              </div>
              ...
              {selectedProduct.addons.length > 0 && (
                <div>
                  <h3 className={`font-bold text-sm mb-2 ${muted2}`}>Adicionais</h3>
              ...
              <div>
                <label className={`text-sm ${muted}`}>Observação</label>
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Ex: Sem cebola..."
                  className={`w-full mt-1 p-2.5 rounded-xl text-sm resize-none outline-none ${inputCls}`} rows={2} />
              </div>

              <div className="flex items-center gap-3 pt-1">
                <div className={`flex items-center gap-2 rounded-xl p-1 border ${card}`}>
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white/5"><Minus className="h-4 w-4" /></button>
                  <span className="font-black w-5 text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white/5"><Plus className="h-4 w-4" /></button>
                </div>
                <button onClick={addToCart} className="flex-1 bg-gradient-to-r from-[#ff9607] to-[#ff0080] text-black py-3.5 rounded-xl font-bold">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2.5">
                {cart.map((item, index) => {
                  ...
                  const price = item.product.isPromotion && item.product.promotionPrice ? item.product.promotionPrice : item.product.price;
                  return (
                    <div key={index} className={`flex gap-3 p-2.5 ${card} rounded-xl`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.selectedAddons.length > 0 && <p className={`text-xs ${muted} truncate`}>+ {item.selectedAddons.map(a => a.name).join(', ')}</p>}
                        {item.note && <p className={`text-xs ${muted} truncate`}>Obs: {item.note}</p>}
                        <p className="text-[#ff9607] font-bold text-sm mt-0.5">R$ {((price + addonsTotal) * item.quantity).toFixed(2)}</p>
                      </div>
              ...
              <Link href={`/${slug}/checkout`} className="block w-full bg-gradient-to-r from-[#ff9607] to-[#ff0080] text-black py-3.5 rounded-2xl font-bold text-center text-lg">
                Finalizar pedido
              </Link>
            </div>
          </div>
